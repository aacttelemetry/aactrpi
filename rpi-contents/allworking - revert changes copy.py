    ...
    if states.s2 != None:
        while True:
            ...
            if len(c) < 2:
                continue
            if c[1] == "TEMP":
                assign["TEMP"] = "states.s2"
                print("/USB1 = TEMP")
                break
            elif c[1] == "GPS":
                assign["GPS"] = "states.s2"
                print("/USB1 = GPS")
                break
            elif c[1] == "DISP":
                assign["DISP"] = "states.s2"
                print("/USB1 = DISP")
                break
    else:
...
async def cget():
    ...
    try:
        if not connecting:
            ...
            print("socket bound to port %states.s"%(port))
...
async def hello(websocket, path):
    ...
    while True:
        ...
        temp_data = eval("str(%states.s.read_until())"%assign["TEMP"]) if assign["TEMP"] else "--|TEMP|--|--"
        ...
        disp_data = eval("str(%states.s.read_until())"%assign["DISP"]) if assign["DISP"] else "--|DISP|--|--|--|--|--|--|--|--"
        gps_data = eval("str(%states.s.read_until())"%assign["GPS"]) if assign["GPS"] else "--|GPS|--|--|--|--"
        ...
        try:
            ...
            failed_fitbit += 1
            print("Seems the fitbit connection was lost. %states.s failed connections."%failed_fitbit)
        ...
        try:
            ...
            print("Something else happened: %states.s"%e)
